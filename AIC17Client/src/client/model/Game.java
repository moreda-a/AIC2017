...
public class Game implements World {
	...
	private HashMap<Integer, Entity> infoMap = new HashMap<>(); // This field is
																// not usefull
																// for user
	...
	public void changeStrategy(BeetleType type, CellState right, CellState front, CellState left, Move newStrategy) {
		Event event = new Event("s", new Object[] { type.ordinal(), right.ordinal(), front.ordinal(), left.ordinal(),
				newStrategy.ordinal() });
	...
	public void handleInitMessage(Message msg) {
		...
		for (int i = 0; i < Beetles.size(); i++) {
			...
			Cell theChosenCell = map.getCelli(cellX, cellY);
	...
	private void itemAlter(ArrayList<Integer> changes) {
		...
		Cell targetCelli = cells[x][y];

		targetCelli.receiveInfo(theChosenCell.getItem());
		idMap.put(id, targetCelli);
		if (targetCelli.getItem().getId() == theChosenCell.getItem().getId()) {
	...
	private void moveBeetle(ArrayList<Integer> changes) {
		...
		case 1:
			...
			Cell targetCelli = map.getCelli(cellX, cellY);
			targetCelli.receiveInfo(theChosenInfo);
			idMap.put(id, targetCelli);
			if (targetCelli.getBeetle().getId() == theChosenCell.getBeetle().getId()) {
				theChosenCell.clear();
			}
	...
	private void beetleAlter(ArrayList<Integer> changes) {
		...
		Cell targetCelli = map.getCelli(newX, newY);
		theChosenInfo.setColor(color);
		theChosenInfo.setSick(sick);

		targetCelli.receiveInfo(theChosenInfo);
		idMap.put(id, targetCelli);
		if (targetCelli != theChosenCell) {
			if (targetCelli.getBeetle().getId() == theChosenCell.getBeetle().getId()) {
				theChosenCell.clear();
			}
